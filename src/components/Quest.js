...
function Quest() {
  ...
  const [muted, setMuted] = useState(
    parseInt(levelId) === 1 && parseInt(questId) === 1
  );
  ...
  useEffect(() => {
    ...
    const questCompleted = user.answers
      ? user.answers[`l${levelId}q${questId}`]
      : false;
    if (questCompleted) {
    ...
    const hintAvailable = user.hints
      ? user.hints[`l${levelId}q${questId}`]
      : false;
    if (hintAvailable) {
  ...
  const validate = async (e) => {
    ...
    const accuracy = await fetch(
      `https://spotihunt-backend.vercel.app/api/validate-answer?answer=${encodeURI(
        answer.replace(/[^a-zA-Z0-9 ]/g, "")
      )}&level=${levelId}&quest=${questId}`
    ).then((data) => {
      if (data.status === 200) {
        return data.text();
      } else {
    ...
    if (accuracy === -1) {
      toast.error(
        "Looks like something went wrong. Please try again. If the problem persists, contact us."
      );
      ...
      db.collection("users")
        ...
        .then(() => {
          setLoading(false);
          if (
            parseInt(levelId) < vars.levels &&
            parseInt(questId) === vars.quests
          ) {
      ...
      toast.warning(
        `Sorry, that's not the answer we were looking for. But you're answer is ${(
          parseFloat(accuracy) * 100
        ).toFixed(2)}% right. Try again!`
      );
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {levelComplete && (
        <LevelCompleted levelId={levelId} time={completedTime} />
      )}
      {questIsUnlocked ? (
        <>
          <form className="quest__box" onSubmit={(e) => validate(e)}>
            <p className="quest__question">
              {levelId}/{questId}
              {}
              {questions && questions[`l${levelId}`][`q${questId}`]}
            </p>
            {hint && !questCompleted && (
              <motion.p
                initial={{ scale: 0.75, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                variants={{ type: "tween", duration: 0.3 }}
                className="quest__hint"
              >
                <img src={HintIcon} />
                <span>{hint}</span>
              </motion.p>
            )}
            {questCompleted && userAnswers && (
              <motion.p
                initial={{ scale: 0.75, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                variants={{ type: "tween", duration: 0.3 }}
                className="quest__hint accuracy"
              >
                <b>Accuracy: </b>
                {(parseFloat(userAnswers[1]) * 100).toFixed(2)}%
              </motion.p>
            )}
            <span className="quest__answer">
              <input
                ...
                value={userAnswers ? userAnswers[0] : answer}
            ...
            {questCompleted && (
              <motion.img
                ...
                initial={{ scale: 2 }}
                animate={{ scale: 1 }}
                variants={{ type: "spring", duration: 1 }}
                className="completed"
              />
            )}
          </form>
          <span
            ...
            onClick={() => setTimeout(() => setMuted(!muted), 100)}
          >
