...
function Sidebar() {
  ...
  const getHint = async () => {
    ...
    const hintAvailable = user.hints
      ...
      : false;
    const alreadyAnswered = user.answers
      ? user.answers[`l${levelId}q${questId}`]
      : false;
    if (!hintAvailable && !alreadyAnswered) {
      toast.info("Getting your hint...", { autoClose: 1500 });
      fetch(
        `https://spotihunt-backend.vercel.app/api/get-hint?level=${levelId}&quest=${questId}&used=${usedHints}`
      ).then((data) => {
        if (data.status === 200) {
    ...
    <div className="sidebar">
      ...
      <div className="sidebar__levels">
        {levelsArray.map((lvl) => {
          if (lvl == 1) {
            return (
              <NavLink activeClassName="active" to="/lvl/1">
                {isLevelUnlocked(lvl + 1) ? (
                  <CheckCircleRounded />
                ) : (
                  <AccessTimeRounded />
                )}{" "}
            ...
            return isLevelUnlocked(lvl) ? (
              <NavLink activeClassName="active" to={`/lvl/${lvl}`}>
                {isLevelUnlocked(lvl + 1) ? (
                  <CheckCircleRounded />
                ) : (
                  <AccessTimeRounded />
      ...
      <p className="sidebar__info">
        New levels unlock on completion of current level.
      </p>
      <div className="sidebar__hints">
        <h4 style={{ marginBottom: "0.5rem" }}>Feeling stuck!?</h4>
        <p>
          You are given a total of only three hints for the entire contest. Use
          wisely.
        </p>
        <div className="sidebar__hintsImg">
          {hintsArray.map((h, index) => (
            <button onClick={getHint} disabled={usedHints > index}>
              <img src={usedHints > index ? BulbOff : BulbOn} />
            </button>
          ))}
        </div>
      </div>
